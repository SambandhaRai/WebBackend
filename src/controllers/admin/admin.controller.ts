...
let adminUserSerice = new AdminUserService();
...
export class AdminUserController {
    async createUser(req: Request, res: Response){
        try {
            ...
            const newUser = await adminUserSerice.createUser(parsedData.data);
    ...
    async getUserById(req: Request, res: Response){
        try{
            ...
            const user = await adminUserSerice.getUserById(userId);
    ...
    async getAllUser(req: Request, res: Response){
        try{
            const users = await adminUserSerice.getAllUsers();
            return res.status(200).json(
    ...
    //             });
    //         }
    //         const updatedUser = await adminUserSerice.updateOneUser(userId, parsedData.data);
    //         return res.status(200).json(
    ...
    async deleteOneUser(req: Request, res: Response) {
        try{
            const userId = req.params.id;
            const user = await adminUserSerice.deleteOneUser(userId);
            return res.status(200).json(
